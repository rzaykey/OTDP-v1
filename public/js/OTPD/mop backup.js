...
$(document).ready(function () {
    $.ajax({
        url: LinkGetEmployeeOperator, // Laravel route
        type: "GET",
...
async function fetchDataTable() {
    try {
        const response = await fetch(LinkGetMOPDataSimple);
        ...
        populateFilter(
            "filterUnit",
            getUniqueValues(sampleData.data, "unit_category")
        );
...
function populateFilter(id, values, displayKey = null) {
    ...
    values.forEach((item) => {
        ...
        if (displayKey === "jde_with_name") {
            opt.value = item.jde_no;
            opt.textContent = `${item.jde_no} - ${item.employee_name}`;
        } else {
            opt.value = item;
...
function getUniqueJDEWithNames(data) {
    ...
    const map = new Map();
    data.forEach((item) => {
        if (item.jde_no && item.employee_name) {
            map.set(item.jde_no, item.employee_name);
        }
    });

    return Array.from(map.entries()).map(([jde_no, employee_name]) => ({
        jde_no,
        employee_name,
    }));
}
function updateTable() {
    ...
    const filtered = sampleData.data.filter(
        (item) =>
            ...
            (!filters.unit || item.unit_category === filters.unit)
    );
    ...
    filtered.forEach((row) => {
        ...
        tr.innerHTML = `
                ...
                <td>${row.equipment_type1}</td>
                <td>${row.total_point}</td>
                <td>${row.mop_bulanan_grade}</td>
            `;
...
async function loadDataRequirement() {
    ...
    const response = await fetch(`${LinkGetMOPDataSimple}?${params.toString()}`);
